...
void actual_fare(void){				
	...
	if(rounded_count_for_distance*speed_auto>1.5)		//check if distace travelled>1.5 kms
	{
		temp_val=rounded_count_for_distance*speed_auto-1.5;
	}
	else{
		temp_val=0;
	}

	fare_for_travel = 21 + 14.2*temp_val;		//fare for travelling
...
{	P1=0x0F;			//initialize port 1
	...
	while(P1^0==0)         //auto is available, since trip has not started, resetting all variables
	{
	...
	while(P1^0==1){             	  //auto currently hired
		if(P1^3==1)      			 //auto is waiting
		{	valu_disp_ind = 1;
			IE = 0x82;
			TR0 = 1;
			while(P1^3==1 && P1^0==1){ // till auto is waiting and it is hired, waiting charges increase
		...
		while(P1^3==0 && P1^0==1) //till auto is NOT waiting and it is hired, travelling charges increase
		{
			if(((overflow_count_for_distance/40)-(pre_ocfd/40))==1) // if time elapsed is one sec
			{   
				if(P1^2==0 && P1^1==0){	//find value of speed
					valu_disp_ind = 2;
					speed_auto = 0.1;}
				if(P1^2==0 && P1^1==1){	
					valu_disp_ind = 3;
